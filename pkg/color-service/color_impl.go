...
import (
	...
	colorsvc "github.com/kyawmyintthein/twirp-api-gateway-poc/rpc/color"
)

type colorHandler struct{}

func NewColorHandler() colorsvc.ColorService {
	return &colorHandler{}
}
...
func (h *colorHandler) GetRandomColor(ctx context.Context, getRandomColorRequest *colorsvc.GetRandomColorRequest) (*colorsvc.GetRandomColorResp, error) {
	var colors []string
	for i := 0; i < int(getRandomColorRequest.Count); i++ {
		color := mapRandomKeyGet(Map).(string)
		colors = append(colors, color)
	}
	return &colorsvc.GetRandomColorResp{Colors: colors}, nil
}
...
func (h *colorHandler) GetRGBColor(ctx context.Context, getRGBColorRequest *colorsvc.GetRGBColorRequest) (*colorsvc.GetRGBColorResp, error) {
	color, ok := Map[getRGBColorRequest.Name]
	if !ok {
		return nil, fmt.Errorf("color not found")
	}
	return &colorsvc.GetRGBColorResp{
		Red:   int64(color.R),
		Green: int64(color.G),
		Blue:  int64(color.B),
	}, nil
